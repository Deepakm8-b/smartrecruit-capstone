cat > /Applications/XAMPP/xamppfiles/htdocs/smartrecruit/candidate_profile.php << 'ENDFILE'
<?php
session_start();
require_once 'db.php';
...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
        $action = $_POST['action'];
        $allowedActions = ['Shortlisted', 'Rejected', 'Accepted'];
    ...
    $stmt = $pdo->prepare('
        SELECT a.application_id, a.status, a.applied_date, a.resume_used,
               s.student_id, s.full_name, s.phone, u.email, s.university, s.degree, s.gpa,
               j.job_title, j.company
        FROM applications a
        JOIN jobs j ON a.job_id = j.job_id
        JOIN students s ON a.student_id = s.student_id
        JOIN users u ON s.user_id = u.user_id
        WHERE a.application_id = ? AND j.organisation_id = ?
    ');
...
.header { background: white; padding: 24px; border-radius: 8px; border: 1px solid #e5e7eb; margin-bottom: 24px; display: flex; justify-content: space-between; }
.header-left { flex: 1; }
...
.answer-text { color: #6b7280; font-size: 13px; line-height: 1.5; }
</style>
</head>
<body>
...
<div class="container">
    ...
    <div class="header">
        <div class="header-left">
            ...
            <span class="status-badge status-<?php echo strtolower($application['status']); ?>"><?php echo $application['status']; ?></span>
        </div>
